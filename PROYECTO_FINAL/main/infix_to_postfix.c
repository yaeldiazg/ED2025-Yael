...
char* infix_to_postfix(List *tokens) {
    ...
    for (ListNode *elmt = list_head(tokens); elmt != NULL; elmt = list_next(elmt)) {
        ...
        history_printf("Token actual: %s\n", t);

        if (isdigit(t[0]) || (t[0]=='-' && isdigit(t[1]))) {
            strcat(result, t);
            strcat(result, " ");
            history_printf("Añadido a resultado: %s\n", result);
        } else if (t[0]=='(') {
            stack_push(&s, t);
            history_printf("Push '(' a pila\n");
        } else if (t[0]==')') {
            while (stack_peek(&s)[0] != '(') {
                char *op = stack_pop(&s);
                strcat(result, op);
                strcat(result, " ");
                history_printf("Pop de pila: %s -> resultado: %s\n", op, result);
                free(op);
            }
            // Sacamos el '(' de la pila
            char *op = stack_pop(&s);
            free(op);
            history_printf("Pop de '(' de pila\n");
        } else {
            while (stack_peek(&s) && precedence(stack_peek(&s)[0]) >= precedence(t[0])) {
                char *op = stack_pop(&s);
                strcat(result, op);
                strcat(result, " ");
                history_printf("Pop de pila por precedencia: %s -> resultado: %s\n", op, result);
                free(op);
            }
            stack_push(&s, t);
            history_printf("Push operador a pila: %s\n", t);
        }
    }
    ...
    while (stack_peek(&s)) {
        ...
        strcat(result, op);
        strcat(result, " ");
        history_printf("Pop final de pila: %s -> resultado: %s\n", op, result);
        free(op);
    }
